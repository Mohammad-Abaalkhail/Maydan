...
HOST="72.61.84.181"
SSH_USER="root"
REPO_URL="${REPO_URL:-https://github.com/user/almaydan-ya-7maidan.git}"  # USER MUST PROVIDE
...
if [ -z "\$BASIC_USER" ] || [ -z "\$BASIC_PASS" ]; then
  ...
  [ -f nginx/auth/.htpasswd ] && BASIC_PASS=\$(openssl passwd -apr1 < /dev/null 2>/dev/null || echo "check_manually") || true
fi
